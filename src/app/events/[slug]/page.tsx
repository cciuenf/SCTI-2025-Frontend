...
const SlugEventPage = async (props: Props) => {
  ...
  const cardCls = cn(
    "group flex flex-col items-center justify-center p-4 h-32",
  ...
  const valueCls = "text-lg font-bold text-secondary text-center";
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center min-h-screen h-full">
      ...
      <section
        ...
        className="mx-auto mt-6 mb-8 max-w-2xl px-2"
      >
        <dl className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className={cardCls} tabIndex={0} role="group" aria-label="Participantes">
            ...
            <dd className={valueCls}>{eventRes.data.participant_count || 0}</dd>
          </div>
        </dl>
      </section>
      <EventManagementActions isEventCreator={isEventCreator} event={eventRes.data}/>
      <section className="max-w-3xl mx-auto px-4 container my-12">
