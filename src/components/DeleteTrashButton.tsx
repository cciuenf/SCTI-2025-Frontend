...
type Props = {
  ...
  position: string;
  deleteFunction: (uniqueParam: string) => Promise<{ success: boolean }>;
};
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Trash
          className={cn(
            "w-7 h-7 p-1 rounded-md cursor-pointer bg-destructive text-popover hover:text-destructive hover:bg-transparent duration-300 absolute",
            position
          )}
        />
      </PopoverTrigger>
      <PopoverContent side="left" className="w-80 h-28" sideOffset={12}>
        <div className="w-full rounded-md flex flex-col items-center justify-around">
          <p className="text-sm">Tem certeza que deseja apagar esse evento?</p>
